<x-layout>
        ...
        <section class="md:col-span-3">
                    ...
                    @can('update', $job)
                        <div class="flex space-x-3 ml-4">
                            ...
                            <form method="POST" action="{{ route('jobs.destroy', $job->id) }}"
                                onsubmit="return confirm('Are you sure that you want to delet this job?')">
                                @csrf
                                @method('DELETE')
            ...
            <x-modal buttonText="Apply Now">
              <h3 class="text-lg font-semibold mb-4">
                        ...
                        <form action="{{route('applicant.store', $job->id)}}" method="POST" enctype="multipart/form-data" class="">
                            @csrf
                            ...
                            <x-inputs.text id="contact_email" name="contact_email" label="Contact Email" required />
                            <x-inputs.text-area id="message" name="message" label="Message" />
                            <x-inputs.text id="location" name="location" label="Location" />
                            <x-inputs.file id="resume" name="resume" label="Upload Your Resume (pdf)" required />
    ...
    <aside class="bg-white rounded-lg shadow-md p-3">
        ...
        @else
            <form
                action="{{ !auth()->user()->bookmarkedJobs()->where('job_id', $job->id)->exists() ? route('bookmarks.store', $job->id) : route('bookmarks.destroy', $job->id) }}"
                ...
                <button
                    ...
                    {{ !auth()->user()->bookmarkedJobs()->where('job_id', $job->id)->exists() ? 'Bookmark Listing' : 'Remove Bookmark' }}
                </button>
            </form>
        @endguest
    </aside>
